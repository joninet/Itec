...
def deudasMayor(lista, deuda):
    contador = 0
    try:
        for rec in lista:
            dni, nombre, deudaPesos, localidad = rec.split("#")
            if int(deudaPesos) >= float(deuda):
                contador += int(deudaPesos)
        print(
            f"El total de deuda acumulada de los grandes deudores es de ${contador}")
    except:
        print("Error - Ingresar solo numeros enteros")


deudasMayor(datos, 90000)
ingresoDeuda = input("Deudas mayor que: ")
deudasMayor(datos, ingresoDeuda)


def dniClientes(lista, dni):
    try:
        print("Los apellidos de los clientes cuyos DNI sean mayores a", dni, "son:")
        for rec in lista:
            dniNum, nombre, deudaPesos, localidad = rec.split("#")
            if int(dniNum) >= float(dni):
                nom, apell = nombre.split(" ")
                print(apell)
    except:
        print("Error - Ingresar solo numeros enteros")


dniClientes(datos, 40000000)
ingresoDni = input("Ingresar el numero minimo de DNI: ")
dniClientes(datos, ingresoDni)


def buscarLocalidad(lista, localidad):
    contador = 0
    for rec in lista:
        buscaLoc = rec.find(localidad)
        if buscaLoc != -1:
            contador += 1
    if contador != 0:
...
buscarLocalidad(datos, "La Carlota")
localidad = input("Ingresar localidad: ")
buscarLocalidad(datos, localidad)
