...
class Programador(Empleado):
    ...
    def get(self):
        return {'Nombre': self.nombre, 'Salario': self.sueldo, 'Proyecto': self.proyecto, 'Lenguaje': self.lenguaje}
...
class Empresa:
    ...
    def mostrarTodo(self):
        ...
        for p in self.listaProgramadores:
            print(f"{p.get()['Nombre']}. Salario: {p.get()['Salario']}. Sistema: {p.get()['Proyecto']}. Lenguaje: {p.get()['Lenguaje']}")
...
for d in datos:
    empresa.agEmp(*d) # el argumento es un desagregador
empresa.mostrarTodo()